...
import com.mt.filelibrary.widget.AutoFitTextureView;
import com.mt.filelibrary.widget.CaptureButton;
...
public class CameraCaptureRecordFragment extends BaseFragment implements OnCameraCaptureListener {
    private CameraOpenType mode; // 拍摄模式
    ...
    public CameraCaptureRecordFragment() {
        CameraHelper.Builder mBuilder = CameraHelper.getBuilder();
        this.mode = mBuilder.getCameraOpenType();
        this.duration = mBuilder.getDuration();
    }
    ...
    public View onCreateView(@NonNull LayoutInflater inflater, @Nullable ViewGroup container, @Nullable Bundle savedInstanceState) {
        ...
        if (mode == CameraOpenType.TAKE_PHOTO) {
            viewTextInfo.setText("轻触拍照");
        } else if (mode == CameraOpenType.TAKE_VIDEO) {
            viewTextInfo.setText("长按摄像");
        } else if (mode == CameraOpenType.TAKE_PHOTO_IMAGE) {
        ...
        // 切换闪光灯
        viewSplashMode.setOnClickListener(v -> capture.enableFlashLight());

        // 切换摄像头
        viewSwitch.setOnClickListener(v -> capture.switchCamera());
    ...
    public void onCapturePhoto(String photoPath) {
        ...
        getActivity().getSupportFragmentManager()
                ...
                .replace(R.id.camera_capture_main_framelayout,
                        new CameraCapturePreviewFragment(Util.Const.MODE_PHOTO, photoPath, 0),
    ...
    public void onCaptureRecord(String filePath) {
        ...
        getActivity().getSupportFragmentManager()
                .beginTransaction()
                .replace(R.id.camera_capture_main_framelayout,
                        new CameraCapturePreviewFragment(Util.Const.MODE_VIDEO, filePath, realDuration),
                        CameraCapturePreviewFragment.class.getSimpleName())
                .addToBackStack(null)
                .commit();
    }
